...
export function findModelReferences(config, { providerId = '', modelId = '' } = {}) {
  ...
  collectReference(references, 'agent.acceptanceReview.model', agent?.acceptanceReview?.model);
...
export function rewriteModelReferences(config, { providerRenames = new Map(), modelRenames = new Map() } = {}) {
  ...
  if (agent) {
    ...
    if (agent.acceptanceReview?.model !== undefined) agent.acceptanceReview.model = renameRef(agent.acceptanceReview.model, providerRenames, modelRenames);
  }
