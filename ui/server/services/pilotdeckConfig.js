...
function validateAcceptanceReview(config, errors) {
  const value = config.agent?.acceptanceReview;
  if (value === undefined) return;
  const fail = (message) => errors.push(`agent.acceptanceReview: ${message}`);
  if (!isRecord(value)) { fail('must be an object'); return; }
  for (const key of Object.keys(value)) {
    if (!['enabled', 'model', 'maxTurns', 'timeoutMs'].includes(key)) fail(`unknown field ${key.slice(0, 80)}`);
  }
  if (value.enabled !== undefined && typeof value.enabled !== 'boolean') fail('enabled must be a boolean');
  if (value.model !== undefined && (typeof value.model !== 'string' || !splitModelRef(value.model) || !resolveModel(config, value.model, { allowMissing: true }))) {
    fail('model must reference a configured provider/model; omit it to follow the main Agent');
  }
  for (const [key, min, max] of [['maxTurns', 1, 8], ['timeoutMs', 1000, 180000]]) {
    if (value[key] !== undefined && (!Number.isInteger(value[key]) || value[key] < min || value[key] > max)) fail(`${key} must be an integer from ${min} to ${max}`);
  }
}
...
function finalizeBootstrapPlaceholder(config) {
  ...
  if (referencesRemovedProvider(config?.agent?.acceptanceReview?.model)) {
    delete config.agent.acceptanceReview.model;
  }
