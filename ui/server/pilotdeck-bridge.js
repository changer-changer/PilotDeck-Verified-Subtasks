...
/**
 * Terminal subagent acceptance outcomes, kept so a following
 * `subagent_completed` cannot overwrite the semantic 验收通过 / 验收未通过
 * label with the generic 已完成 / 执行失败. Bounded per entry
 * (one pre-formatted label) and bounded in size with FIFO eviction; entries
 * are deleted when consumed by `subagent_completed` or invalidated by a new
 * `subagent_started` for the same child.
 */
const subagentAcceptanceOutcomes = new Map();
const MAX_TRACKED_SUBAGENT_ACCEPTANCE = 256;
const MAX_ACCEPTANCE_ISSUES_SHOWN = 3;
const MAX_ACCEPTANCE_ISSUE_TEXT_CHARS = 160;
...
function createSubagentStatusFrames(event, base) {
    ...
    const acceptanceOutcome = rememberSubagentAcceptance(event.event, base.sessionId, subagentId, detail);
    let status = normalizeSubagentStatus(event.event, detail);
    if (acceptanceOutcome?.failed) status = 'failed';
    ...
    let activityDetail = formatSubagentActivityDetail(event.event, detail, status);
    if (acceptanceOutcome) {
        // Preserve the semantic acceptance label (验收通过 / 验收未通过 + issues)
        // instead of the generic 已完成 / 执行失败 fallback.
        activityDetail = acceptanceOutcome.label;
    }
...
function formatSubagentActivityDetail(eventName, detail, status) {
    ...
    if (eventName === 'subagent_acceptance') {
        return formatSubagentAcceptanceDetail(detail);
    }
    if (status === 'failed') {
        return '执行失败';
    }
...
function normalizeSubagentStatus(eventName, detail) {
    ...
    if (eventName === 'subagent_acceptance') {
        if (detail.phase === 'accepted') return 'completed';
        if (detail.phase === 'rejected') return 'failed';
        return 'running';
    }
    return 'running';
}

function formatSubagentAcceptanceDetail(detail) {
    const attempt = Number(detail?.attempt);
    switch (detail?.phase) {
        case 'reviewing':
            return '模型正在核对任务与实际交付';
        case 'repairing':
            return Number.isFinite(attempt) && attempt > 0
                ? `正在局部修复（第 ${attempt} 次）`
                : '正在局部修复';
        case 'accepted': {
            const model = typeof detail?.review?.model?.model === 'string' ? detail.review.model.model.slice(0, 80) : '';
            const evidence = Array.isArray(detail?.review?.evidence) ? detail.review.evidence.length : 0;
            return model ? `验收通过 · ${model} 复核${evidence ? ` · 独立读取 ${evidence} 份文件` : ''}` : '验收通过';
        }
        case 'rejected': {
            const issueText = formatAcceptanceIssues(detail?.issues);
            return issueText ? `验收未通过：${issueText}` : '验收未通过';
        }
        case 'validating':
        default:
            return '正在验收';
...
function formatAcceptanceIssues(issues) {
    if (!Array.isArray(issues) || issues.length === 0) return '';
    const lines = [];
    for (const issue of issues.slice(0, MAX_ACCEPTANCE_ISSUES_SHOWN)) {
        if (!issue || typeof issue !== 'object') continue;
        const path = typeof issue.path === 'string' ? issue.path.trim() : '';
        const message = typeof issue.message === 'string' ? issue.message.trim() : '';
        if (!path && !message) continue;
        let text = [path, message].filter(Boolean).join(': ');
        if (text.length > MAX_ACCEPTANCE_ISSUE_TEXT_CHARS) {
            text = `${text.slice(0, MAX_ACCEPTANCE_ISSUE_TEXT_CHARS)}…`;
        }
        lines.push(text);
    }
    if (lines.length === 0) return '';
    return lines.length < issues.length
        ? `${lines.join('；')}（共 ${issues.length} 个问题）`
        : lines.join('；');
}

function rememberSubagentAcceptance(eventName, sessionId, subagentId, detail) {
    const activityKey = `${sessionId || ''}:${subagentId}`;
    if (eventName === 'subagent_started') {
        // A fresh activity for the same child invalidates a stale outcome.
        subagentAcceptanceOutcomes.delete(activityKey);
        return null;
    }
    if (eventName === 'subagent_acceptance') {
        const phase = detail?.phase;
        if (phase !== 'accepted' && phase !== 'rejected') return null;
        if (subagentAcceptanceOutcomes.size >= MAX_TRACKED_SUBAGENT_ACCEPTANCE) {
            const oldest = subagentAcceptanceOutcomes.keys().next();
            if (!oldest.done) subagentAcceptanceOutcomes.delete(oldest.value);
        }
        subagentAcceptanceOutcomes.set(activityKey, {
            label: formatSubagentAcceptanceDetail(detail),
            failed: phase === 'rejected',
        });
        return null;
    }
    if (eventName !== 'subagent_completed') return null;
    const stored = subagentAcceptanceOutcomes.get(activityKey);
    if (!stored) return null;
    subagentAcceptanceOutcomes.delete(activityKey);
    if (detail?.aborted === true) return null;
    // No false green: a rejected acceptance stays failed even when the
    // completion reports success, and an accepted acceptance still defers
    // to an actually-failed completion.
