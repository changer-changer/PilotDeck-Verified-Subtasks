...
export function clearSubagentDefaultForRemovedProvider(
  ...
  let next = parsed?.providerId === providerId ? patch(config, ["agent", "subagents", "default"], "inherit") : config;
  if (splitModelRef(next.agent?.acceptanceReview?.model)?.providerId === providerId) next = patch(next, ["agent", "acceptanceReview", "model"], undefined);
  return next;
}
...
export function clearSubagentDefaultForRemovedModel(
  ...
  const parsed = splitModelRef(config.agent?.subagents?.default);
  let next = parsed?.providerId === providerId && parsed.modelId === modelId ? patch(config, ["agent", "subagents", "default"], "inherit") : config;
  const reviewer = splitModelRef(next.agent?.acceptanceReview?.model);
  if (reviewer?.providerId === providerId && reviewer.modelId === modelId) next = patch(next, ["agent", "acceptanceReview", "model"], undefined);
  return next;
}
...
export function rewriteProviderRefs(
  ...
  const reviewerModel = rewriteProviderRef(next.agent?.acceptanceReview?.model, oldProviderId, newProviderId);
  if (reviewerModel !== next.agent?.acceptanceReview?.model) next = patch(next, ["agent", "acceptanceReview", "model"], reviewerModel);
