...
export type AgentRuntimeDependencies = {
  ...
  /**
   * Trusted host-registered subtask validators for opt-in verified subtask
   * execution. The host owns registration; acceptance contracts may only
   * reference names present here (never model-supplied commands). Passed
   * through into forked subagent dependencies so nested sessions inherit.
   */
  subtaskValidators?: SubtaskValidators;
  /**
   * Optional second-layer model reviewer for verified subtasks. Invoked by
   * `SubAgentSession.runWithAcceptance` only AFTER the deterministic schema /
   * host checks pass and only when at least one shared model turn remains.
   * Hosts build one via `createModelSubtaskReviewer`; absent → verified
