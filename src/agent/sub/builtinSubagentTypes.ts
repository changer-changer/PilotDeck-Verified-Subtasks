...
export function buildSubagentSystemPrompt(definition: SubagentDefinition, outputFormat: "summary" | "json" = "summary"): string {
  if (outputFormat === "summary") return `${SHARED_PREFIX}\n\n${definition.systemPromptSuffix}`;
  const prefix = SHARED_PREFIX.split("\nOutput format (mandatory;")[0]!.replace(
    "6. The final assistant message MUST follow the output format below verbatim.",
    "6. The final assistant message MUST be one JSON value matching the supplied acceptance contract.",
  );
  return `${prefix}\n\n${definition.systemPromptSuffix}\n\nOutput format for this run: return only the JSON value declared in the acceptance contract. This replaces prose-report formatting requirements. Complete the actual work before reporting.`;
}
