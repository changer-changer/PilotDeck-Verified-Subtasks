...
import type { SubtaskAcceptanceResult } from "../sub/acceptance/types.js";
import {
...
export class AgentLoop {
  ...
  async *run(input: AgentLoopInput): AsyncGenerator<AgentEvent, AgentLoopRunResult, unknown> {
    ...
    while (true) {
      ...
      try {
        const toolContext = this.createToolContext(input, messages, executedRequest ?? decision);
  ...
  private createToolContext(
    input: AgentLoopInput,
    messages: CanonicalMessage[],
    dispatchModel: { provider: string; model: string } = this.config,
  ): PilotDeckToolRuntimeContext {
    ...
    return {
      ...
      subagent: this.buildSubagentForkApi(input, messages, dispatchModel),
  ...
  private buildSubagentForkApi(
    ...
    return {
      ...
      fork: async ({ definitionId, directive, subagentId, toolCallId, abortSignal, timeoutMs, acceptance }) => {
        ...
        const subSession = new SubAgentSession({
          ...
          parentConfig: {
            ...this.config,
            acceptanceReviewModel: this.config.acceptanceReviewModel ?? {
              provider: dispatchModel.provider, model: dispatchModel.model,
            },
        ...
        // A rejected acceptance contract is a structured failure: the child
        // report (markdown + usage) survives, but lifecycle / transcript must
        // not present it as successful.
        const acceptanceRejected = report.acceptance?.status === "rejected";
        const summary = acceptanceRejected
          ? `${report.markdown}\n\n${formatAcceptanceRejection(report.acceptance!)}`
          : report.markdown;
        ...
        await transcriptHooks?.recordSubagentCompleted?.({
          ...
          errored: errored || acceptanceRejected,
        });
        await this.dispatchLifecycle(input, "SubagentStop", {
          subagentId,
          subagentType: def.id,
          success: !errored && !acceptanceRejected,
        });
        this.dependencies.eventEmitter?.({
          type: "subagent_completed",
          sessionId: input.sessionId,
...
/** Bounded, human-readable rejection trailer for subagent transcript summaries. */
function formatAcceptanceRejection(acceptance: SubtaskAcceptanceResult): string {
  const latest = acceptance.attempts[acceptance.attempts.length - 1];
  const issues = (latest?.issues ?? [])
    .slice(0, 10)
    .map((issue) => `- [${issue.path}] ${issue.code}: ${issue.message}`)
    .join("\n");
  return [
    `[acceptance rejected] stopReason=${acceptance.stopReason} attempts=${acceptance.attempts.length} repairs=${acceptance.repairs}`,
