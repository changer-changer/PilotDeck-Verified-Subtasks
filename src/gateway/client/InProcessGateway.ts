    ...
    case "subagent_acceptance":
      return [{
        ...
        detail: {
          ...
          ...(event.review ? { review: {
            status: event.review.status, model: event.review.model, summary: event.review.summary.slice(0, 400),
            evidence: event.review.evidence.slice(0, 32).map(path => path.slice(0, 1024)),
            turns: event.review.turns, usage: event.review.usage, durationMs: event.review.durationMs,
          } } : {}),
          ...(event.issues?.length ? { issues: limitAcceptanceIssues(event.issues) } : {}),
        },
      }];
    default:
      return [];
...
const MAX_ACCEPTANCE_ISSUES = 8;
const MAX_ACCEPTANCE_ISSUE_TEXT_CHARS = 200;

function limitAcceptanceIssues(
  issues: { path?: string; code?: string; message?: string }[],
): Record<string, string>[] {
  return issues.slice(0, MAX_ACCEPTANCE_ISSUES).map((issue) => ({
    path: typeof issue?.path === "string" ? issue.path.slice(0, MAX_ACCEPTANCE_ISSUE_TEXT_CHARS) : "",
    code: typeof issue?.code === "string" ? issue.code.slice(0, MAX_ACCEPTANCE_ISSUE_TEXT_CHARS) : "",
    message: typeof issue?.message === "string"
      ? issue.message.slice(0, MAX_ACCEPTANCE_ISSUE_TEXT_CHARS)
      : "",
  }));
}
