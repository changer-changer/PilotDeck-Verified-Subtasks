    ...
    try {
      ...
      if (output.error) {
        await this.dispatchLifecycle("PostToolUseFailure", tool.name, call.id, executeInput, context, {
          ...
          isInterrupt: output.error.code === "tool_aborted",
        });
        ...
        const result: PilotDeckToolErrorResult = {
          type: "error", toolCallId: call.id, toolName: tool.name,
          error: output.error, content: output.content,
          supplementalMessages: output.supplementalMessages,
          metadata: mergeMetadata(output.metadata, previewLimit.metadata ? { previewLimit: previewLimit.metadata } : undefined),
