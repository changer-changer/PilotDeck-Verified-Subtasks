...
export type PilotDeckSubagentForkApi = {
  ...
  fork(args: {
    ...
    timeoutMs?: number;
    /** Opt-in verified-subtask contract. Validated by the child session preflight. */
    ...
    /** Structured acceptance verdict when a contract was requested. */
...
export type PilotDeckToolExecutionOutput<Output = unknown> = {
  content: PilotDeckToolResultContent[];
  /** Expected, structured delivery failure that must preserve its diagnostic content. */
  error?: import("./errors.js").PilotDeckToolError;
