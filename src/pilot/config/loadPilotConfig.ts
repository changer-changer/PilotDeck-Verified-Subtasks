...
function parseAgent(
  ...
  const acceptanceReview = parseAcceptanceReview(rawAgent.acceptanceReview, modelConfig, diagnostics);
...
function parseAcceptanceReview(
  value: unknown,
  modelConfig: ReturnType<typeof parseModel>,
  diagnostics: PilotConfigDiagnostic[],
): PilotAgentConfig["acceptanceReview"] {
  if (value === undefined) return undefined;
  const fail = (message: string): never => { throw new PilotConfigError("CONFIG_ACCEPTANCE_REVIEW_INVALID", `agent.acceptanceReview: ${message}`); };
  if (!isRecord(value)) return fail("must be an object");
  for (const key of Object.keys(value)) if (!["enabled", "model", "maxTurns", "timeoutMs"].includes(key)) fail(`unknown field ${key.slice(0, 80)}`);
  ...
  const integer = (key: string, fallback: number, min: number, max: number): number => {
    ...
    if (typeof number !== "number" || !Number.isInteger(number) || number < min || number > max) return fail(`${key} must be an integer from ${min} to ${max}`);
    return number;
  };
  const model = value.model === undefined ? undefined : parseAgentModelSelection(value.model, "agent.acceptanceReview.model", modelConfig, diagnostics);
  return { enabled: value.enabled !== false, ...(model ? { model } : {}), maxTurns: integer("maxTurns", 4, 1, 8), timeoutMs: integer("timeoutMs", 60000, 1000, 180000) };
}
