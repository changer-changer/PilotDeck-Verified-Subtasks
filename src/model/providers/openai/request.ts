...
export function buildOpenAIRequest(
  ...
  const body: OpenAIRequestBody = {
    ...
    // BigModel rejects the OpenAI metadata field (HTTP 400 / 1210).
    // Keep it in the canonical request and local traces, but omit it on this wire.
    metadata: request.metadata && acceptsRequestMetadata(provider)
      ? Object.fromEntries(
...
function acceptsRequestMetadata(provider?: ProviderConfig): boolean {
  try { return new URL(provider?.url ?? "").hostname !== "open.bigmodel.cn"; }
  catch { return true; }
}
