...
import {
  ACCEPTANCE_CONFIG_ENV,
  loadAcceptanceArtifactResolver,
  type ArtifactAcceptanceResolver,
} from "../agent/sub/acceptance/artifactRegistry.js";
import {
...
class ProjectRuntimeRegistry {
  ...
  /**
   * Opt-in artifact acceptance validators (PILOTDECK_ACCEPTANCE_CONFIG).
   * Loaded and snapshotted exactly once per registry so later manifest edits
   * cannot weaken checks of running sessions. `undefined` keeps legacy
   * behavior when the operator did not opt in.
   */
  private readonly acceptanceArtifacts?: ArtifactAcceptanceResolver;
  ...
  constructor(private readonly options: ProjectRuntimeRegistryOptions) {
    ...
    this.acceptanceArtifacts = loadAcceptanceArtifactResolver(options.env[ACCEPTANCE_CONFIG_ENV]);
  }
  ...
  private async prepareSessionRuntime(context: GatewaySessionContext) {
    ...
    // -- opt-in artifact acceptance validators (PILOTDECK_ACCEPTANCE_CONFIG) --
    // Host-configured validators are bound to this EXACT project root. They
    // ride on baseDependencies so ordinary sessions and runtime recreations
    // both inherit them (nested subagents inherit via parentDependencies).
    const acceptanceValidators = this.acceptanceArtifacts?.(projectRoot);
    const hasAcceptanceValidators =
      acceptanceValidators !== undefined && Object.keys(acceptanceValidators).length > 0;
    if (hasAcceptanceValidators) {
      // Expose the available validator IDs + {artifact,result} convention in a
      // COPY of this session's agent tool description. The shared project
      // registry and the global tool definition are never mutated.
      const agentTool = sessionTools.get("agent");
      if (agentTool) {
        if (sessionTools === runtime.tools) {
          sessionTools = runtime.tools.clone();
        }
        sessionTools.replace({
          ...agentTool,
          description: appendAcceptanceValidatorNotice(
            agentTool.description,
            Object.keys(acceptanceValidators),
          ),
        });
      }
    }

    const reviewConfig = runtime.snapshot.config.agent.acceptanceReview;
    const baseDependencies: CreateAgentSessionOptions["dependencies"] = {
      ...
      ...(reviewConfig?.enabled !== false ? { subtaskReviewer: createModelSubtaskReviewer({
        ...
        maxTurns: reviewConfig?.maxTurns, timeoutMs: reviewConfig?.timeoutMs,
      }) } : {}),
      lifecycle,
      now: this.options.now,
...
/**
 * Per-session copy of the `agent` tool description advertising the host
 * acceptance validators available for this project. Purely additive text —
 * the shared tool definition, its aliases and permissions stay untouched, and
 * acceptance remains an explicit model choice, never a mandatory gate.
 */
function appendAcceptanceValidatorNotice(description: string, validatorIds: readonly string[]): string {
  const ids = validatorIds
    .filter((id) => typeof id === "string" && id.length > 0)
    .map((id) => id.slice(0, 80))
    .slice(0, 32)
    .join(", ");
  if (!ids) return description;
  return [
    description,
    "",
    `Host acceptance validators registered for this project: ${ids}.`,
    'Each validator expects the subagent\'s final JSON value shaped { "artifact": "<exact artifact filename>", "result": <validated data> }, where "artifact" names the checked file and "result" deep-equals its saved JSON content.',
    "These checks run only when you explicitly pass an acceptance contract naming them via acceptance.validators; they are optional and never enforced automatically.",
  ].join("\n");
}
