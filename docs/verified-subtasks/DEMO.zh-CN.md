...
完整操作、讲稿与答辩见 [现场手册](FIELD-GUIDE.zh-CN.md)。
...
5. 完成后打开当前项目 Memory，查看“子任务验收经验”：生产任务样本、成功与失败、修复次数、模型来源。当前不自动修改契约，也不宣称学习收益。
6. 核对另外三份报表的文件与修改时间保持不变。解释收益是把问题留在失败子任务内解决；模型复核自身会增加调用与时延。
...
比赛 GLM-5.3 开启验收记忆的最新完整预演用时 215.664 秒，四份交付通过、简报两次局部修复，四条观察进入原生记忆；现场建议预留 5–7 分钟。历史无记忆预演 160.474 秒不能用来估计开关记忆的净影响，因为两次修复路径不同。先用约一分钟说明错误与验收设置，执行中展示实际读取和问题反馈，最后查看修复产物与成功报表；该时长只作排练参考，网络与模型响应会变化。
